...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    # Stage 1
    x = KL.ZeroPadding2D((8, 8))(input_image)
    x = KL.Conv2D(64, (8, 8), strides=(2, 2), name='conv1', use_bias=True)(x)
    x = BatchNorm(name='bn_conv1')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    C1 = x = KL.MaxPooling2D((4, 4), strides=(2, 2), padding="same")(x)
    # Stage 2
    ...
    for i in range(block_count):
        x = identity_block(x, 3, [256, 256, 1024], stage=4, block=chr(99 + i), train_bn=train_bn)
    C4 = x
    # Stage 5
    x = conv_block(x, 3, [256, 256, 1024], stage=5, block='a', train_bn=train_bn)
    x = identity_block(x, 3, [256, 256, 1024], stage=5, block='b', train_bn=train_bn)
    C5 = x = identity_block(x, 3, [256, 256, 1024], stage=5, block='c', train_bn=train_bn)
